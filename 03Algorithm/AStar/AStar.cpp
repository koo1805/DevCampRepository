...
std::vector<Position> AStar::FindPath(const Position& startPosition, const Position& goalPosition, std::vector<std::vector<int>>& grid)
{
    ...
    // 시작/목표 위치가 grid기준에서 문제 없는 위치 값인지 확인
    ...
    // 시작노드의 비용 계산 및 openList에 추가해 탐색 시작
    ...
    while (!openList.empty())
    {
        ...
        {
            // 이동 경로 제작 후 반환
            return ConstructionPath(currentNode);
        }
        ...
        // 현재 위치를 기준으로 주변(8방향)의 이웃 노드를 탐색
...
std::vector<Position> AStar::ConstructionPath(Node* destination)
{
    ...
    // 루프가 종료되면 path에는 반대 방향의 경로가 저장됨
...
float AStar::CalculateHeuristic(const Position& current, const Position& goal) const
{
    ...
    int straightDistance = std::min(diffX, diffY) - diagonalDistance;
...
bool AStar::IsDiagonalBlocked(const Position& current, const Direction& direction, const std::vector<std::vector<int>>& grid) const
{
    ...
    {
        // 대각선 방향의 x,y성분은 모두 0이 아님
        return false;
    }

    // 대각선으로 이동하려는 새로운 위치 x성분과 y성분을 분해
    int sideX = current.x + direction.x;
    int sideY = current.y + direction.y;

    // 대각선 이동 성분 위치 중 하나라도 장애물이 있으면 이동 불가
...
{
    // 두 노즈 모두 null이 아니고, 두 노드의 위치가 같은지 비교
