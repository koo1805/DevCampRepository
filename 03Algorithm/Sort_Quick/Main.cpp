...
int Partition(int* array, int left, int right, Comparer comparer = LessEqual)
{
	...
	int low = left + 1;			// 왼쪽에서 오른쪽으로 이동 || 피벗보다 큰 값을 찾는데 사용
	int high = right;			// 오른쪽에서 왼쪽으로 이동 || 피벗보다 작은 값을 찾는데 사용

	// low와 high인덱스가 교차할 때까지 진행
	while (low <= high)
	{
	...
	// 피벗 위치 반환 - 배열의 인덱스
	// high를 반환하는 이유는 앞에서 초기 피벗 선택 위치와 high 위치를 교환했기 때문
	return high;
}
...
void BubbleSort(int* array, int length, Comparer comparer = Less)
{
	...
	{
		for (int jx = 0; jx < length - 1 - jx; ++jx)
		{
