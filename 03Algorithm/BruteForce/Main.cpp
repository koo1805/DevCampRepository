...
// 문자열에서 특정 패턴 검색하는 함수
// ex) yourname 에서 rna찾기
int FindPatternIndex(const char* text, const char* pattern)
{
	//int testLength = StringLength(text);
	int textLength = static_cast<int>(strlen(text));
	int patternLength = static_cast<int>(strlen(pattern));
...
int main()
{
	...
	int resultIndex = FindPatternIndex("yourname", "rna");
	std::cout << "문자: " << resultIndex << "\n";
}